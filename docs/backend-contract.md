...
- **Background items**: parse `sfltool dumpbtm` (works without root on macOS 13+; it usually answers in 5 s but needs up to a minute on a busy Mac, so use a 60 s limit; when it fails return an empty list and the stderr text as the error). Records start with ` #<n>:` under a `Records for UID <uid>` header. Return the records of the current uid, of uid 0 and of uid -2. Skip the `Embedded Item Identifiers` sub-lists. Read-only: never call `resetbtm`.
